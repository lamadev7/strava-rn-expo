...
import { acceptPoint, haversineM, toTrackPoint, type TrackPoint } from './geo';

export const RECORDING_TASK = 'trace-recording';

/**
 * TECH_SPEC §5.1/§5.2 — the ingest path must not depend on React being alive.
 * This task is a pure DB writer: batch in → filter → append track_points →
 * bump activities.distance_m. The UI is a reader (useLiveQuery). Runs in the
 * app's JS runtime for both foreground and background location deliveries.
 */
TaskManager.defineTask<{ locations: LocationObject[] }>(RECORDING_TASK, async ({ data, error }) => {
  ...
  for (const location of data.locations) {
    ...
    if (!acceptPoint(point, previous, active.type)) continue;
